...
The discrepancy ratio important is because it is connected to the edge expansion $\Phi(S)$ of a set $S$. This can be seen by choosing $f$ to be the indicator vector for the set, so \begin{equation}
	f_v=\begin{cases}
	1, & \text{if } v\in S\\
	0, & \text{otherwise}
	\end{cases}.
